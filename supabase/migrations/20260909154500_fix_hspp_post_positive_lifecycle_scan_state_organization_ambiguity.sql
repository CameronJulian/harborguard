/*
 * Correct PL/pgSQL organization_id ambiguity in the post-positive
 * lifecycle scan-state compare-and-swap function.
 *
 * The function RETURNS TABLE(... organization_id ...), making
 * organization_id a PL/pgSQL output variable. The final UPDATE must
 * therefore qualify the relation column explicitly.
 *
 * This migration intentionally preserves the existing CAS,
 * contention, cursor-validation and authorization behavior.
 */
-- HarborGuard HSPP
...
begin
   ...
   */
  update
    public.hspp_post_positive_lifecycle_scan_states as scan_state
  set
    previous_cursor_positive_assessed_at =
      v_state.cursor_positive_assessed_at,
