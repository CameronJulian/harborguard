-- B7490-Q14AG35AS35
--
-- Correct PL/pgSQL name ambiguity in
-- persist_hspp_open_assembly_membership_relation().
--
-- RETURNS TABLE output names such as organization_id and assembly_id are
-- PL/pgSQL variables. Every SQL relation below therefore uses an explicit
-- table alias and fully qualified column references.
--
-- No lifecycle semantics or authority are changed.
...
is
  'B7490-Q14AG35AS35 correction: same AS26A immutable OPEN-child B11A2 persistence authority, with every SQL table-column reference explicitly alias-qualified so RETURNS TABLE output variables cannot create PL/pgSQL name ambiguity.';