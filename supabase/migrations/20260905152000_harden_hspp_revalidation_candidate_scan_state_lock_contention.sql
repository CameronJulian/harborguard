 ...
 * HarborGuard HSPP
 *
 * Harden post-positive revalidation candidate scan-state CAS scheduling
 * contention.
 *
 * This forward migration replaces the blocking per-Q14p advisory lock
 * with a non-blocking transaction-scoped try-lock.
 *
 * CONTENDED is scheduling backpressure only. It grants no semantic,
 * lifecycle, execution-lease, Q14p existence, organization, subject,
 * or durable cursor-state authority.
 */
...
begin
   ...
   */
  /*
   * Contention is scheduling backpressure only.
   *
   * Failure to acquire this transaction-scoped advisory lock does not
   * establish that Q14p exists and does not observe organization,
   * historical C identity, or durable cursor state.
   */
