...
begin
  ...
  --
  -- Exact idempotent child recovery has already returned above.
  -- Therefore only a NEW descendant reconstruction reaches here.
  --
  -- Historical membership remains immutable, but Q14ab may record
  -- that its CURRENTLY EFFECTIVE lifecycle membership has ceased.
  -- A ceased parent membership must never be derived as RETAINED
  -- into a newly-created descendant.
  --
  -- Q14h already holds the exact historical parent FOR UPDATE.
  -- Q14ab serializes cessation against that parent using KEY SHARE.
  -- The guard therefore observes the serialized lifecycle decision
  -- before any NEW descendant membership is persisted.
  -- ----------------------------------------------------------
