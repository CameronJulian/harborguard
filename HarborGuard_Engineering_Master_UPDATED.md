...
## Intelligence Source Registry Enforcement for HERE Traffic Flow

**Completed:** 2026-08-01  
**Branch:** feature/expanded-incident-taxonomy  
**Commit:** 57966d4 — Enforce source registry for HERE Traffic Flow

### Objective

Ensure the existing HERE Traffic Flow integration obeys the centralized
intelligence_sources registry without rebuilding the flow integration,
changing traffic scoring, or modifying API response structures.
...
The shared uildTrafficIntelligence() service now:

- loads the here_traffic source configuration;
- checks enabled;
- checks pproved_for_ingestion;
- skips the HERE Traffic Flow API request when the source is disabled or
  unapproved;
- preserves the existing empty-flow fallback behavior;
- exposes a clear registry-disabled warning;
- preserves congestion, delay, traffic-risk, prediction, and response logic.
...
- 
pm run build passed.
- Next.js generated 119 of 119 static pages.
- Enabled test:
  - flow count: 20
  - average congestion: 82
  - average delay: 21
  - no warning
- Disabled test:
  - flow count: 0
  ...
  - average delay: 0
  - warning: HERE Traffic Flow is disabled by the intelligence source registry.
- Restored test:
  - flow count: 20
  - average congestion: 82
  - average delay: 21
  - no warning
- Registry was restored to:
  - enabled = true
  - pproved_for_ingestion = true

### Result

The here_traffic registry entry now controls both:

1. HERE incident ingestion.
2. HERE live Traffic Flow requests.

Disabling or unapproving HERE prevents both data paths without requiring a
redeployment.

### Roadmap Status

- HERE Traffic Incidents: complete
- TomTom Traffic Incidents: complete
- HERE Traffic Flow: complete
- Traffic-provider registry enforcement: complete
- HERE Traffic Flow registry enforcement: complete
- Weather integration: complete
- Official road closures: blocked pending machine-readable source access
- Crime intelligence: source suitability investigation remains open
- Fleet telemetry fusion: next unblocked live-intelligence milestone
- Dashcam AI fusion: pending
