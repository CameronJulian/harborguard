...
import {
  ...
  type RoutePredictionThresholdEvaluation,
} from "@/lib/fleet/analyzeRoutePredictionThresholds";

type RoutePredictionThresholdEvaluationRow = {
...
export async function GET(req: Request) {
  try {
    ...
    let evaluationsQuery = supabase
      ...
      .select(
        "predicted_risk_score, observed_adverse_event"
      )
    ...
    const rows =
      (data || []) as RoutePredictionThresholdEvaluationRow[];

    const evaluations: RoutePredictionThresholdEvaluation[] =
      rows.map((row) => ({
        predictedRiskScore:
    ...
    const analysis =
      analyzeRoutePredictionThresholds(evaluations);
    ...
    console.error(
      "Route prediction threshold analysis error:",
      error
    );
