...
function timelineTitle(alertType?: string | null) {
  return (alertType || "unknown_alert").replace(/_/g, " ").toUpperCase();
}
...
export async function GET() {
  try {
    ...
    const timelines = alerts.reduce((acc: any, alert: any) => {
      ...
      if (!acc[vehicleId]) {
        acc[vehicleId] = {
          ...
          nickname: alert.vehicle?.nickname || null,
          openAlerts: 0,
          resolvedAlerts: 0,
          maxSeverity: alert.severity || "low",
          escalationScore: 0,
          events: [],
        };
      }
      ...
      acc[vehicleId].escalationScore += alert.is_resolved ? weight : weight * 2;

      if (weight > severityWeight(acc[vehicleId].maxSeverity)) {
        acc[vehicleId].maxSeverity = alert.severity || "low";
      }

      acc[vehicleId].events.push({
        id: alert.id,
        title: timelineTitle(alert.alert_type),
    ...
    const timelineList = Object.values(timelines).sort(
      (a: any, b: any) => b.escalationScore - a.escalationScore
    );
