              ...
              {telemetryReviewPerformance.byAlertType.map((entry) => (
                ...
                >
                  ...
                  >
                    ...
                    <div>
                      Confirmed evidence median:{" "}
                      {formatTelemetryEvidenceStrength(
                        entry.alertType,
                        entry.evidenceStrength.confirmedMedian
                      )}
                    </div>
                    <div>
                    ...
                    <div>
                      False-positive evidence median:{" "}
                      {formatTelemetryEvidenceStrength(
                        entry.alertType,
                        entry.evidenceStrength.falsePositiveMedian
                      )}
                    </div>
                  </div>
                </div>
        ...
        <div style={{ ...cardStyle, padding: 26 }}>
          ...
          {routeSoftCapShadowEvidenceLoading ? (
            ...
            <>
              ...
              <div className="mt-6">
                <h3 className="text-sm font-semibold text-slate-100">
                  By vehicle and UTC evidence day
                </h3>

                <div className="mt-3 space-y-2">
                  {routeSoftCapShadowEvidence.byVehicleUtcDay.length === 0 ? (
                    <p className="text-sm text-slate-400">
                      No vehicle-and-day-linked shadow evidence yet.
                    </p>
                  ) : (
                    routeSoftCapShadowEvidence.byVehicleUtcDay.map((item) => (
                      <div
                        key={`${item.vehicleId}:${item.utcDay}`}
                        className="flex items-center justify-between gap-4 text-sm"
                      >
                        <span className="min-w-0 truncate text-slate-300">
                          {item.vehicleId} · {item.utcDay}
                        </span>
