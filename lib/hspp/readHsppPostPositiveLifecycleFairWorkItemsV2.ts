...
export const HSPP_POST_POSITIVE_LIFECYCLE_FAIR_WORK_READER_VERSION =
  "hspp-post-positive-lifecycle-fair-work-reader-v2" as const;

export const HSPP_POST_POSITIVE_LIFECYCLE_FAIR_WORK_RPC =
  "read_hspp_post_positive_lifecycle_fair_work_items_v2" as const;
 ...
 */
export async function readHsppPostPositiveLifecycleFairWorkItemsV2({
  supabase,
  organizationId,
  limit,
  ...
  if (workItems.length === 0) {
    if (cursorAdvance !== null) {
      throw new Error(
        "Empty fair lifecycle work must not expose cursor advancement.",
      );
    }
  } else {
    if (cursorAdvance === null) {
      throw new Error(
        "Non-empty fair lifecycle work requires a cursor-advance proposal.",
      );
    }
    ...
    if (!finalWorkItem) {
      throw new Error(
        "Fair lifecycle selected page unexpectedly lost its final work item.",
      );
    }


    if (
      finalWorkItem.positiveCheckpointId !==
        cursorAdvance.proposedCursor.positiveCheckpointId ||
      Date.parse(
        finalWorkItem.positiveAssessedAt,
      ) !==
        Date.parse(
      ...
      throw new Error(
        "Fair lifecycle proposed cursor does not identify the final selected work item.",
      );
    }
  }
