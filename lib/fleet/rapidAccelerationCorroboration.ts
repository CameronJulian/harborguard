...
type RapidAccelerationAlertRow = {
  id: string;
...
export type RapidAccelerationCorroborationParams = {
  supabase: any;
...
export type NearbyRapidAccelerationMatch = {
  alertId: string;
  vehicleId: string;
  latitude: number;
  longitude: number;
...
export type RapidAccelerationCorroborationResult = {
  ...
  nearbyAlerts: NearbyRapidAccelerationMatch[];
...
export async function findRapidAccelerationCorroboration({
  supabase,
  organizationId,
  currentVehicleId,
  latitude,
...
}: RapidAccelerationCorroborationParams): Promise<RapidAccelerationCorroborationResult> {
  ...
  const nearbyAlerts: NearbyRapidAccelerationMatch[] =
    (
      (recentAlerts || []) as RapidAccelerationAlertRow[]
    )
      .map(
        (
          alert
        ): NearbyRapidAccelerationMatch | null => {
      ...
      .filter(
        (
          alert
        ): alert is NearbyRapidAccelerationMatch =>
          alert !== null
      );
