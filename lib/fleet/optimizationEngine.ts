...
export async function buildFleetOptimization(supabase: any, organizationId: string) {
  ...
  const trafficCenter = (locationsResult.data || []).find((location: any) => location.latitude && location.longitude);

  const trafficResult = await buildTrafficIntelligence(supabase, organizationId, {
  ...
  const candidates = (vehiclesResult.data || []).map((vehicle: any) => {
    ...
    const finalScore = isBusy ? Math.max(0, score - 15) : score;
