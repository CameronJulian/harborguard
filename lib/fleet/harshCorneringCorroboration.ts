...
export type HarshCorneringCorroborationParams = {
  supabase: any;
...
export type NearbyHarshCorneringMatch = {
  alertId: string;
  vehicleId: string;
  latitude: number;
  longitude: number;
...
export type HarshCorneringCorroborationResult = {
  ...
  nearbyAlerts: NearbyHarshCorneringMatch[];
...
export async function findHarshCorneringCorroboration({
  supabase,
  organizationId,
  currentVehicleId,
  latitude,
...
}: HarshCorneringCorroborationParams): Promise<HarshCorneringCorroborationResult> {
  ...
  const nearbyAlerts: NearbyHarshCorneringMatch[] =
    (
      (recentAlerts || []) as HarshCorneringAlertRow[]
    )
      .map(
        (
          alert
        ): NearbyHarshCorneringMatch | null => {
      ...
      .filter(
        (
          alert
        ): alert is NearbyHarshCorneringMatch =>
          alert !== null
      );
